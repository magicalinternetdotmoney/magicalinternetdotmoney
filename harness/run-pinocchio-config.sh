...
echo "==> deploy"; solana program deploy target/deploy/leverage_engine_pinocchio.so --program-id target/deploy/leverage_engine_pinocchio-keypair.json -u "$FORK_URL" -k "$DEPLOYER" 2>&1 | tail -1
TEST="${1:-harness/tests/pinocchio-config.ts}"
echo "==> test $TEST"; ANCHOR_PROVIDER_URL="$FORK_URL" ANCHOR_WALLET="$DEPLOYER" yarn run ts-mocha -p ./tsconfig.json -t 1000000 "$TEST"
